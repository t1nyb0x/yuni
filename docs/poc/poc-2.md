...
> **【重要・2026-08-31 に判明した穴】検証に使うモーションは、製品で実際に使う想定のものにすること。**
>
> 当初 Mixamo の「脚を組んで太ももを高く上げる」座り姿勢で測っていた。太ももが深く食い込むため**厳しめの良い検証材料だと考えたが、これは誤りだった。**
>
> **エモートは利用者と我々が選べる**（F-16-7）。任意のポーズに耐える必要はない。使う予定のないポーズで破綻したことを根拠にソルバの優劣を判断すると、**判断材料そのものが間違う。**
>
> 実際、この厳しいポーズでの破綻を根拠に「SPCR では 2/3 のモデルに対応できない」という結論へ傾きかけた。**測り直しが要る。**
>
> 使うべきモーション: 膝を揃えて座る、体育座りや脚組みは避ける、太ももと胴の角度は 90 度程度まで。
...
> **実測（2026-08-31）— Karin で自前構成が通った。ADR-0005 の方式が動く。**
>
> ```
> FBX Transform の解決: 362 件
> コライダ 6 個すべて正しい親ボーンへ（LegD_L / LegD_R / Chest / Hips / Head ×2）
> 代替: Skirt (根 14 本 / Point 70 / コライダ 3) <- Magica Mesh Cloth_Skirt
> ```
>
> **MeshCloth で作られたモデルに対し、衣装ボーンから BoneCloth を構成できた。**
>
> 検証できたこと:
>
> | 項目 | 結果 |
> |---|---|
> | 衣装ボーンの発見（Humanoid 以外 + 接頭辞 + 輪判定） | ✅ `Skirt1_1_L`〜`Skirt7_1_R` の 14 本を発見 |
> | 制御用ボーンの除外 | ✅ `ToeTipIK_L/R` と `WaistControl_L/R` は輪の条件で落ちた |
> | 作者の指定を手がかりに使う | ✅ MeshCloth の `colliderList`（Hips + LegD_L/R）をそのまま流用 |
> | チェーン深さの違いへの耐性 | ✅ Chifuyu は Point 84（14×6）、Karin は 70（14×5）。同じ処理で通った |
>
> **作者がどの方式でクロスを作っていても関係ない**（D-10）ことが、実データで確認できた。
>
> **残るのは見た目の判定だけである。** 穏やかな座りモーションで破綻しないかを見る（2 節の注意書き）。
