...
- 状態: **実施中（Chifuyu で Step 1〜4 完了。貫通の解消を確認。判定には残り 2 体が必要）**
...
> **実測（2026-08-31）— Chifuyu で貫通が解消した。3 体中 1 体。**
>
> 変換の結果:
>
> | 項目 | 結果 |
...
> | | 変換前（基準点） | 変換後 |
> |---|---|---|
> | スカート | 硬い円錐のまま静止 | 太ももに押しのけられて変形 |
> | 太もも | **スカート面を突き抜けていた** | **スカートの縁から自然に出ている** |
> | 裾 | 脚と交差 | 脚を避けて外側へ流れる |
>
> **判定は目視である。** 貫通の有無は絵でしか判定できない（1 節）。ギズモを消した同一アングルで基準点と比較した。
>
> **これは 3 体中 1 体にすぎない。** 合格基準は Chifuyu / Karin / Rikka すべてである。1 体で判断すると、moca が PMX で踏んだ「モデルごとの手当てが際限なく必要になる」構造を見落とす。
>
> **残る観察点**: 裾がやや硬く板状に見える。物理パラメータはサンプル(kohaku)の値を借りただけで Chifuyu 向けに詰めていない。**貫通の有無と見た目の自然さは別の判定であり、前者のみ達成した状態。**
