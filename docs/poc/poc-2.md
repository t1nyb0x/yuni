...
> **実測（2026-08-31）— 3 体の構成を比較した。決定的な差が出た。**
>
> Karin と Rikka のプレハブを unitypackage から直接取り出し、同じ抽出器にかけた。**Unity へインポートせずに構造を比較できた。**
>
> | モデル | スカートの実装 | SPCR へ変換 |
> |---|---|---|
> | Chifuyu | **BoneCloth**（ボーン駆動、roots 14） | ✅ 可能 |
> | Karin | **MeshCloth**（頂点駆動）+ Virtual Deformer | ❌ **SPCR に相当物が無い** |
> | Rikka | **MeshCloth** + Virtual Deformer | ❌ 同上 |
>
> **2/3 のモデルは、SPCR が持たない方式でスカートを作っている。**
>
> 作者の設定自体は 3 体とも完結している。MeshCloth はちゃんとコライダを参照していた（Karin: Hips + LegD_L/R、Rikka: LegD_L/R）。**問題は移し先に概念が無いことだけである。**
>
> なお Karin / Rikka にもスカートのボーンは存在する（`Skirt1_1_L` 〜 `Skirt7_1_R` の 14 本。Chifuyu と同じ構成）。作者はボーンがあるうえで MeshCloth を選んでいる。
>
> **【抽出器のバグを 2 回踏んだ】**
>
> 当初「Karin と Rikka はスカートのクロスを持っていない」「太もも・腰のコライダが未参照」と結論したが、**どちらも誤りだった。** 抽出器が `Bone Cloth` しか拾っておらず、`Mesh Cloth` と `Virtual Deformer` を無視していたためである。
>
> **知らないコンポーネントを黙って落とすと、存在しないことにしてしまう。** 抽出器を修正し、変換できないものも `unconvertible` として必ず記録するようにした。**F-18-5 のプレビューが提示すべきものの筆頭がこれである。**
>
> 副次的な確認: **Rikka の LegD コライダは `axis: 1`（Y）で、Chifuyu / Karin の `axis: 0`（X）と違う。** カプセルの向きの補正を軸ごとに実装しておいたのが効いた。決め打ちにしていれば Rikka で壊れていた。
